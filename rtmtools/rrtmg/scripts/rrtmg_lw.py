...
Each input parameter\'s description below is copied from the official
RRTMG instructions from AER.

Input is divided into 3 main sections:
    1. Input for INPUT_RRTM
...
if IATM == 0:
      ...
      ******  these records applicable only if RRTATM not selected (IATM=0)  ******
      ...
      PROVIDE RECORDS 2.1.1 through 2.1.3 for all NLAYRS layers in a file at PATH_atmpro
      '''
      PATH_atmpro = None

      
      
if IATM == 0 and IXSECT == 1:
      '''
      *****************************************************************************
      ******  these records applicable only if RRTATM not selected (IATM=0)  ******
      ****************  and cross-sections ARE selected (IXSECT=1)  ***************
      ...
      PROVIDE RECORDS 2.2.3 through 2.2.5 for NLAYERS layers at file PATH_atmpro
      ''' 
      PATH_atmpro = None
...
if IATM == 1:
	 ...
	 REF_LAT     latitude of location of calculation (degrees)
      ...
      if MODEL == 0:
            ...
            PROVIDE records 3.5 and 3.6.1 to 3.6.N for each of the IMMAX boundaries
            in file at path_atmpro
            '''
            PATH_atmpro = None



if IATM == 1 and IXSECT == 1:
      '''
      *****************************************************************************
      **********  these records applicable if RRTATM selected (IATM=1)  ***********
      ****************  and cross-sections ARE selected (IXSECT=1)  ***************
      ...
      PROVIDE records 3.8.1 to 3.8.N for each of the LAYX boundaries in a file at
      PATH_atmpro
      '''
      PATH_atmpro = None
...
if __name__ == '__main__':
      ...
      content.append(lwin.record_1_2(IAER = IAER,
                                     ...
                                     IMCA = IMCA,
                                     ICLD = ICLD))
      content.append(lwin.record_1_4(TBOUND = TBOUND,
                                         IEMIS = IEMIS,
                                         IREFLECT = IREFLECT,
                                         SEMISS = SEMISS))
      if IDRV == 1:
            content.append(lwin.record_1_4_1(DTBOUND = DTBOUND))
            
      if IATM == 0:
            content.append(lwin.record_2_1(IFORM = IFORM,
                                               NLAYRS = NLAYRS,
                                               NMOL = NMOL))
            content.append(lwin.record_2_1_1to3(NMOL = NMOL,
      ...
      if IATM == 1:
            content.append(lwin.record_3_1(MODEL = MODEL,
                                           IBMAX = IBMAX,
                                           NOPRNT = NOPRNT,
                                           NMOL = NMOL,
                                           IPUNCH = IPUNCH,
                                           ...
                                           CO2MX = CO2MX,
                                           REF_LAT = REF_LAT))
            content.append(lwin.record_3_2(HBOUND = HBOUND,
                                           HTOA = HTOA))
            if IBMAX == 0:
                  content.append(
                        lwin.record_3_3_A(AVTRAT = AVTRAT,
                                          TDIFF1 = TDIFF1,
                                          TDIFF2 = TDIFF2,
                                          ALTD1 = ALTD1,
                                          ALTD2 = ALTD2)
                  ...
                  content.append(
                        lwin.record_3_3_B(IBMAX = IBMAX,
                                          PATH_atmpro = PATH_atmpro)
                        )

            if MODEL == 0:
                  content.append(lwin.record_3_4(IMMAX = IMMAX,
                                                 HMOD = HMOD))
                  content.append(lwin.record_3_5_to_3_6s(NMOL = NMOL,
                                                         IMMAX = IMMAX,
                                                         PATH_atmpro = PATH_atmpro))

      if IATM == 1 and IXSECT == 1:
            content.append(lwin.record_3_7(IXMOLS = IXMOLS,
                                           IPRFL = IPRFL,
                                           IXSBIN = IXSBIN))
            content.append(lwin.record_3_7_1(XSNAME = XSNAME))

            if IPRFL == 0:
                  content.append(lwin.record_3_8(LAYX = LAYX,
                                                 IZORP = IZORP,
                                                 XTITLE = XTITLE))
                  content.append(lwin.record_3_8_1_to_3_8_Ns(IXMOL = IXMOLS,
                                                             LAYX = LAYX,
                                                             PATH_atmpro = PATH_atmpro))
