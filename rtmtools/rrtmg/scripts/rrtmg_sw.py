'''
Script for specifying input for RRTMG_SW (column model) and running it.

This script is adapted directly from the package\'s user instructions
'''

'''
------------------------------------------------------------
Input for INPUT_RRTM 
------------------------------------------------------------
'''
...
IDELM  (0,1) flag for outputting downwelling fluxes computed using
             the delta-M scaling approximation
              = 0  output "true" direct and diffuse downwelling fluxes (default)
              = 1  output direct and diffuse downwelling fluxes
                   computed with delta-M approximation
                   (Note:  The delta-M approximation is always used internally
                   in RRTMG_SW to compute the total downwelling flux at each level.
                   What the IDELM flag determines is whether the components 
                   of the downwelling flux, the direct and diffuse fluxes,
                   that are output are the actual direct and diffuse fluxes (IDELM = 0)
                   or are those computed using the delta-M approximation (IDELM = 1).
'''
IDELM = 0
'''
ICOS   = 0 there is no need to account for instrumental cosine response (default)
       = 1 to account for instrumental cosine response in the computation of
           the direct and diffuse fluxes (unavailable)
       = 2 to account for instrumental cosine response in the computation
           of the diffuse fluxes only (unavailable)
           (Note:  ICOS = 1 and ICOS = 2 requires the presence of the file
           COSINE_RESPONSE, which should  consist of lines containing pairs of numbers
           (ANG, COSFAC), where COSFAC is the instrumental cosine response
           at the angle ANG.)
'''
ICOS = 0
...
JULDAT       Julian day associated with calculation (1-365/366 starting January 1).
             Used to calculate Earth distance from sun.
             A value of 0 (default) indicates no scaling of solar source function
             using earth-sun distance.
'''
JULDAT = None
'''
...
if IATM == 0:
      raise ValueError('Sorry, IATM = 0 is not currently implemented.')
      '''
      *****************************************************************************
      ******  these records applicable only if RRTATM not selected (IATM=0)  ******
      ...
      TZ(L-1)   temperature at bottom of layer L  -
                used by RRTM for Planck Function Calculation
 
      ** NOTE **  PZ(L-1) and TZ(L-1) are only required for the first layer.
      RRTM assumes that these quantites are equal to the top of the previous
      layer for L > 1.
      
      PZ(L)   pressure at top of layer L
 
      TZ(L)   temperature at top of layer L  -
              used by RRTM for Planck Function Calculation
       ...
       WKL(M,L)   column densities or mixing ratios for 7 molecular species
                  (molecules/cm**2)
...
if IATM == 1:
      ...
      ********     these records applicable if RRTATM selected (IATM=1)    ********
      ...
      '''
      MUNITS
      = 0  write molecular column amounts to TAPE7 (if IPUNCH = 1, default)
      ...
      if IBMAX > 0:
            ...
            (8F10.3)
            Provide these in a file at PATH_atmpro
            '''
            PATH_atmpro = 'atmpro_mls70.h5'
      ...
      if MODEL == 0:
            ...
            PROVIDE records 3.5 and 3.6.1 to 3.6.N for each of the remaining
            IMMAX boundaries in a file at path_atmpro
            '''
            PATH_atmpro = 'atmpro_mls70.h5'
...
if __name__ == '__main__':
      import rtmtools.rrtmg.sw.input as swin
      import collections


      content = collections.deque([])

      content.append(swin.record_1_1(CXID = CXID))
      content.append(swin.record_1_2(IAER = IAER,
                                     ...
                                     IDELM = IDELM,
                                     ICOS = ICOS))
      content.append(swin.record_1_2_1(JULDAT = JULDAT,
                                       SZA = SZA,
                                       ISOLVAR = ISOLVAR,
                                       SOLVAR = SOLVAR))
      content.append(swin.record_1_4(IEMIS = IEMIS,
                                     IREFLECT = IREFLECT,
                                     SEMISS = SEMISS))
      
      if IATM == 0:
            content.append(swin.record_2_1(IFORM = IFORM,
                                           NLAYRS = NLAYRS,
                                           NMOL = NMOL))
            content.append(swin.record_2_1_1to3(PATH_atmpro = PATH_atmpro))

      if IATM == 1:
            content.append(swin.record_3_1(MODEL = MODEL,
                                           IBMAX = IBMAX,
                                           NOPRNT = NOPRNT,
                                           NMOL = NMOL,
                                           IPUNCH = IPUNCH,
                                           ...
                                           CO2MX = CO2MX,
                                           REF_LAT = REF_LAT))
            content.append(swin.record_3_2(HBOUND = HBOUND,
                                           HTOA = HTOA))
            
            if IBMAX == 0:
                  conent.append(swin.record_3_3_A(AVTRAT = AVTRAT,
                                                  TDIFF1 = TDIFF1,
                                                  TDIFF2 = TDIFF2,
                                                  ALTD1 = ALTD1,
                                                  ALTD2 = ALTD2))
                  ...
                  content.append(swin.record_3_3_B(IBMAX = IBMAX,
                                                   PATH_atmpro = PATH_atmpro))

            if MODEL == 0:
                  content.append(swin.record_3_4(IMMAX = IMMAX,
                                                 HMOD = HMOD))
                  content.append(
                        swin.record_3_5_to_3_6s(NMOL = NMOL,
                                                IMMAX = IMMAX,
                                                PATH_atmpro = PATH_atmpro)
                        )
